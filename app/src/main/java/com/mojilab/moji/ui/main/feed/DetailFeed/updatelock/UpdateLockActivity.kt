...
class UpdateLockActivity : AppCompatActivity() {
    ...
    fun getCourseData(courseIdx : String){
        ...
        val getdetailFeedResponse = networkService.getCourseData(token, courseIdx)

        getdetailFeedResponse.enqueue(object : retrofit2.Callback<GetCourseResponse>{
            ...
            override fun onResponse(call: Call<GetCourseResponse>, response: Response<GetCourseResponse>) {
                if (response.isSuccessful) {
                    if(response.body()!!.status==200){
                        ...
                        courseUpdateAdapter = CourseUpdateAdapter(applicationContext, imageDatas, requestManager)
